...
# Standard HLL confirmation selected by cross-map test-win AUC. With the
# default three seeds this launches nine jobs. Every map uses sigmoid(T=1), no
# learned calibrator, no Q residual, and its standard-paradigm lattice size.
for seed in "${SEED_LIST[@]}"; do
  next_cuda_device
  run_exp \
    "hll_standard_sigmoid_3s_vs_5z" "hll" "3s_vs_5z" \
    "${seed}" "${NEXT_CUDA_DEVICE}" \
    hll_q_coordinate_fn=sigmoid hll_q_temperature=1.0 \
    hll_q_calibrator_enabled=False
  next_cuda_device
  run_exp \
    "hll_standard_sigmoid_5m_vs_6m" "hll" "5m_vs_6m" \
    "${seed}" "${NEXT_CUDA_DEVICE}" \
    hll_q_coordinate_fn=sigmoid hll_q_temperature=1.0 \
    hll_q_calibrator_enabled=False
  next_cuda_device
  run_exp \
    "hll_standard_sigmoid_MMM2" "hll" "MMM2" \
    "${seed}" "${NEXT_CUDA_DEVICE}" \
    hll_q_coordinate_fn=sigmoid hll_q_temperature=1.0 \
    hll_q_calibrator_enabled=False
done
...
echo
echo "Launched ${#PIDS[@]} jobs (${#SEED_LIST[@]} seeds x 3 experiments)."
